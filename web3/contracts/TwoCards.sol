...
/// @title TwoCards
/// @notice This contract handles the token management and battle logic for the TwoCards game
...
contract TwoCards is ERC1155, Ownable, ERC1155Supply {
    ...
    function getPlayer(address addr) public view returns (Player memory) {
        require(isPlayer(addr), unicode"Người chơi không tồn tại!");
    ...
    function getPlayerToken(
        ...
        require(isPlayerToken(addr), unicode"Game token không tồn tại!");
    ...
    function getBattle(
        ...
        require(isBattle(_name), unicode"Trận đấu không tồn tại!");
    ...
    function registerPlayer(
        ...
        require(!isPlayer(msg.sender), unicode"Người chơi đã được đăng ký"); // Require that player is not already registered
    ...
    function createRandomGameToken(string memory _name) public {
        require(
            !getPlayer(msg.sender).inBattle,
            unicode"Người chơi đang trong trận đấu"
        ); // Require that player is not already in a battle
        require(
            isPlayer(msg.sender),
            unicode"Vui lòng đăng ký người chơi trước"
    ...
    function createBattle(
        ...
        require(!isBattle(_name), unicode"Trận đấu đã tồn tại!"); // Require battle with same name should not exist
    ...
    function joinBattle(string memory _name) external returns (Battle memory) {
        ...
        require(
            ...
            unicode"Trận đấu đã bắt đầu!"
        ); // Require that battle has not started
        require(
            _battle.players[0] != msg.sender,
            unicode"Chỉ người chơi thứ hai mới có thể tham gia trận chiến"
        ); // Require that player 2 is joining the battle
        require(
            !getPlayer(msg.sender).inBattle,
            unicode"Đã sẵn sàng chiến đấu"
        ); // Require that player is not already in a battle
    ...
    function _registerPlayerMove(
        ...
        require(
            _choice == 1 || _choice == 2,
            unicode"Lựa chọn nên là 1 hoặc 2!"
        );
        require(
            _choice == 1 ? getPlayer(msg.sender).playerMana >= 3 : true,
            unicode"Mana không đủ để tấn công!"
        );
    ...
    function _awaitBattleResults(string memory _battleName) internal {
        ...
        require(
            msg.sender == _battle.players[0] ||
                msg.sender == _battle.players[1],
            unicode"Chỉ những người chơi trong trận chiến này mới có thể hành động"
        );

        require(
            _battle.moves[0] != 0 && _battle.moves[1] != 0,
            unicode"Người chơi vẫn cần phải thực hiện lượt"
        );
